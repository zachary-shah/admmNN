...
train_loss = np.zeros((nopt, args.ntrials, args.max_iter)) * np.nan
train_acc = np.zeros((nopt, args.ntrials, args.max_iter)) * np.nan
test_loss = np.zeros((nopt, args.ntrials, args.max_iter)) * np.nan
test_acc = np.zeros((nopt, args.ntrials, args.max_iter)) * np.nan
...
for (k, optimizer_config) in enumerate(optimizer_configs):
    for t in range(args.ntrials):
        ...
        if optimizer_labs[k].startswith("NCVX"):
            ...
            test_loss[k, t] = el
            test_acc[k, t] = ea
            solve_time[k, t] = np.array([0,st,0,0,0]) # only put solve time into u update
