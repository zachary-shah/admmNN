...
- Train / Val loss / accuracy vs iteration number, averaged over n trials
- Solve time, broken down into main ADMM stages (precomputations, u updates, etc)

Run this script to get solve times for 10 iterations, comparing cpu and gpu
"""
...
plt.savefig(os.path.join(args.save_root, "performance-"+base_save_str+".png"))
...
# also save solve time in datatable
solve_data = {
    'optimizer': optimizer_labs,
    'precomputation': solve_times_mean[:,0],
    'u updates': solve_times_mean[:,1],
    'v updates': solve_times_mean[:,2],
    's updates': solve_times_mean[:,3],
    'dual updates': solve_times_mean[:,4],
}
